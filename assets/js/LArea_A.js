	...
	MobileArea.prototype = {
		...
		setGearTooth: function(data) {
			...
			if(l > 0) {
				...
				gearChild[_self.index].style["-webkit-transform"] = 'translate3d(0,' + (-gearVal * 3) + 'rem,0)';
				gearChild[_self.index].setAttribute('top', -gearVal * 3 + 'rem');
		...
		finish: function(e) {
			...
			//alert(_self.trigger.value);
			var param = {
				id: infoid.id,
				address: _self.trigger.value
			}
			HttpAjax(edit_p, param, "json", "");
			_self.close(e);
		},
		close: function(e) {
			e.preventDefault();
